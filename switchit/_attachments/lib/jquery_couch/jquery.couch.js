  ...
  $.extend($.couch, {
    ...
    db: function(name, db_opts) {
      ...
      return /** @lends $.couch.db */{
        ...
        //save attachment given an id, revision and file.
        //in the options
        saveAttachment: function(id,rev,file,options,ajaxOptions){
          ajaxOptions = ajaxOptions || {};
          options = options || {};
          var method = "PUT";
          var db = this;
          //put filename here somewhere
          var fileName = options.filename || file.name;
          var uri = this.uri+encodeDocId(id)+'/'+fileName+'?rev='+rev;
          var xhrPost = new XMLHttpRequest();
          progressReport = function(eventArgs){
            var done = eventArgs.position || eventArgs.loaded,
              total = eventArgs.totalSize || eventArgs.total;
              console.log("xhr progress: "+(Math.floor(done/total*1000)/10) + '%');
          };
          xhrPost.file = file;
          // xhrPost.addEventListener("error", uploadFailed, false);
          // xhrPost.addEventListener("abort", uploadCanceled, false);
          xhrPost.addEventListener("progress",progressReport,false);
          if(xhrPost.upload){
            xhrPost.upload.onprogress = progressReport;
          }
          xhrPost.onreadystatechange = function(eventArgs){
            if( 4 == this.readyState){
              console.log(["xhr upload complete",eventArgs]);
              if(options.ready){
                options.ready(JSON.parse(xhrPost.responseText));
              }
            }
            else{
              console.log(['xhr other ready state',this.readyState,eventArgs]);
            }
          };
          xhrPost.open(method,uri,true);
          xhrPost.send(file);
        },
